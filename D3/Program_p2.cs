...
namespace D2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            char[,] playground = new char[5000, 5000];
            ...
            // retrace routes
            Console.WriteLine("Analyze to list below, find the pair with the smalles sum of steps for the same coordinate");
            for (int w = 0; w < 2; w++)
            {
                // start from center
                int xpos = xstart;
                ...
                foreach (string segment in path)
                {
                    ...
                    for (int i = 0; i < dist; i++)
                    {
                        ...
                        mStepCount++;
                        if ((xpos > (playground.GetLength(0) - 2)) || (ypos > (playground.GetLength(1) - 2))
                            || (xpos < 1) || (ypos < 1)) continue;
                        if (playground[xpos, ypos] == '+')
                        {
                            // intersection found
                            Console.WriteLine("Junction on wire {3:c} x={0},\ty={1}\tSteps: {2}", xpos, ypos, mStepCount, tiles[w]);
                        }
                    }
                }
            }
